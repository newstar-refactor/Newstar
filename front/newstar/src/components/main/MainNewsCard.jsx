...
// 뉴스 이미지
function NewsHeader({ recommendData }) {
  return (
    <div><HeaderImage src={recommendData.imageUrl} alt="news image" /></div>
    )
  }
  
  // 뉴스 내용
  function NewsBody({ recommendData, isLiked, handleLikeButtonClick }) {
    return (
      <div>
        <NewsBodyHeader>
          <div>
            <Tag fontSize={'12px'}>{`# ${recommendData.Bcategory}`}</Tag>
            <Tag fontSize={'12px'}>{`# ${recommendData.Scategory}`}</Tag>
          </div>
          <LikeButton 
            isLiked={isLiked}
        ...
        <NewsContent recommendData={recommendData} />
      </div>
  )
}

// 뉴스 제목, 요약
function NewsContent({ recommendData }) {
  return (
    <div>
      <h2>{recommendData.title}</h2>
      <div>{recommendData.date}</div>
      <br />
      <div>{recommendData.content}</div>
      <a href={recommendData.url}><NewsLink recommendData={recommendData} /></a>
...
// 뉴스 북마크
function NewsLink({ recommendData }) {
  return (
    <NewsLinkWrapper>
      <NewsLinkInfo>
        <div>{recommendData.title}</div>
        <div>{recommendData.content}</div>
        <div>{recommendData.url}</div>
      </NewsLinkInfo>
      <img src={recommendData.imageUrl} alt="news image" height={110} />
    </NewsLinkWrapper>
  )
}
      ...
      <div>
        <Slider {...sliderSettings}>
          {recommendDatas.map((recommendData) => (
            <div key={recommendData.id}>
              <NewsHeader recommendData={recommendData} />
              <NewsBody 
